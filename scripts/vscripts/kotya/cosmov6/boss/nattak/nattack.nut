...
// ▚▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▞
{
    ...
    //FIRE
    {
        function Cast_Fire()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x02 烈火");
            ...
            EntFire("cmd","Command","say 基纳塔克使用了烈火---5", 0);
        }
    }
    //HEAL
    {
        function Cast_Heal()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了治疗");
            ...
            EntFire("cmd","Command","say 基纳塔克治疗了自己---5", 0);
        }
    }
    //WIND
    {
        function Cast_Wind()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x05 狂风");
            ...
            EntFire("cmd","Command","say 基纳塔克使用了狂风---5", 0);
        }
    }
    //SILENCE
    {
        function Cast_Silence()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x0C 沉默");
            ...
            //Звук сайленса
            EntFire("cmd","Command","say 基纳塔克使用了沉默---20", 0);
        }
    }
    //POISON
    {
        function Cast_Poison()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x04 剧毒");
            ...
            EntFire("cmd","Command","say 基纳塔克使用了剧毒---3", 0);
        }
    }
    //MINION
    {
    ...
    //GRAVITY
    {
        function Cast_Gravity()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x0E 黑洞");
            ...
            EntFire("cmd","Command","say 基纳塔克使用了黑洞---5", 0);
        }
    }
    //SHIELD
    {
        ...
        function Cast_Shield()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x0C 护盾");
            EntFire("cmd","Command","say 基纳塔克使用了护盾---5", 0);
    ...
    //FIRENOVA
    {
        ...
        function Cast_FireNova()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x02 火焰新星");
            EntFire("cmd","Command","say 基纳塔克使用了火焰新星,远离平台中间和头顶火球---3", 0);
    ...
    //SUPERATTACK
    {
        ...
        function Cast_SuperAttack()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x02 超级攻击");
            ...
            EntFire("cmd","Command","say 基纳塔克使用了熔岩喷发,找到突起的石头保护自己---15", 0);
    ...
    //ULTIMA
    {
        ...
        function Cast_Ultima()
        {
            ...
            ScriptPrintMessageChatAll(Scan_pref + "\x07基纳塔克\x01 使用了\x02 终极");
            EntFire("cmd","Command","say 基纳塔克使用了终极---15", 0);
...
// ▚▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▞
{
    function Init()
    {
        ...
        EntFire("cmd","Command","say 与基纳塔克的战斗将在10秒后开始", 0);
        EntFire("Credits_Game_Text", "AddOutput", "message 基纳塔克", 0);
    ...
    {
        Stanblock = false;
        ScriptPrintMessageChatAll(Chat_pref + "基纳塔克虚弱了,你现在可以让他晕眩了!");
        EntFire("cmd","Command","say 基纳塔克虚弱了,你现在可以让他晕眩了!---5", 0);
    }
    ...
    function ShowBossStatus()
    {
        ...
        if(!Stanned)
        {
            if(NadeCount >= NadeShow)
            {
                ...
                message += "\n晕眩状态 : " + ((proccent / NadeNeed) * 100) + "%";
            }
        }
        else
        {
            message += "\n晕眩时间 : " + (NadeTickRate).tointeger();
        }
