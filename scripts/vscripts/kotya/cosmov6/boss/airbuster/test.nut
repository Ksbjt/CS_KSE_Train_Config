...
function PreStart()
{
    ...
    EntFire("Credits_Game_Text", "AddOutput", "message 暴风破坏者", 0);
...
{
    Stanblock = false;
    ScriptPrintMessageChatAll(Chat_pref + "暴风破坏者虚弱了,你现在可以让他晕眩了!");
    EntFire("cmd","Command","say 暴风破坏者虚弱了,你现在可以让他晕眩了!---5", 0);
}
...
//        FakeParentArray.push(fakeparent(ent, "C_VFXMuzzleD_a", Vector(0, 0, 0), Vector(0, 200, 0)));
    ...
    function ShowBossStatus()
    {
        ...
        message += "\n\n核心 A: " + GetProccentHP(HP_U, HP_U_TOTAL) + "%";
        message += "\n核心 B: " + GetProccentHP(HP_H, HP_H_TOTAL) + "%";
        message += "\n核心 C: " + GetProccentHP(HP_C, HP_C_TOTAL) + "%";
        message += "\n核心 D: " + GetProccentHP(HP_L, HP_L_TOTAL) + "%";
