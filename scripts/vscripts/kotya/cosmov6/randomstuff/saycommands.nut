//Main
{
    ...
    ::Tifa_pref <- " \x06(\x0E 蒂法 \x06) ➢ \x01"
    ::RedX_pref <- " \x06(\x07 赤红13 \x06) ➢ \x01"
    ::OldMan_pref <- " \x06(\x09 老年人 \x06) ➢ \x01"
    ::ShinraSoldier_pref <- " \x06(\x02 神罗士兵 \x06) ➢ \x01"
...
function Help()
{
    ...
    text = "\x09" + Command_pref + "id [nickname|steam ID]\x01 - 显示自己的\x04 ID";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x09" + Command_pref + "stage\x01 - 显示现在的\x04 关卡";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x09" + Command_pref + "stats [nickname|steam ID|@me|userid] [info|item|perk|hm|zm]\x01 - 显示自己的\x04 信息";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x09" + Command_pref + "transfer [nickname|steam ID|userid] <amount>\x01 - 交易你的\x04 " + Money_pref + "\x01 给其他玩家";
    ScriptPrintMessageChatAll(Chat_pref + text);

    text = "\x09" + Command_pref + "ew|entwatch|hud\x01 - 切换\x04 EntWatch HUD\x01 显示";
    ScriptPrintMessageChatAll(Chat_pref + text);
...
{
    local text = "";
    if(pl.bio_lvl > 0)text += "浮毒 <- ["+pl.bio_lvl+"/"+MaxLevel+"]\n";
    if(pl.ice_lvl > 0)text += "冰 <- ["+pl.ice_lvl+"/"+MaxLevel+"]\n";
    if(pl.poison_lvl > 0)text += "剧毒 <- ["+pl.poison_lvl+"/"+MaxLevel+"]\n";
    if(pl.wind_lvl > 0)text += "风 <- ["+pl.wind_lvl+"/"+MaxLevel+"]\n";
    if(pl.summon_lvl > 0)text += "召唤 <- ["+pl.summon_lvl+"/"+MaxLevel+"]\n";
    if(pl.fire_lvl > 0)text += "火 <- ["+pl.fire_lvl+"/"+MaxLevel+"]\n";
    if(pl.electro_lvl > 0)text += "电 <- ["+pl.electro_lvl+"/"+MaxLevel+"]\n";
    if(pl.earth_lvl > 0)text += "土 <- ["+pl.earth_lvl+"/"+MaxLevel+"]\n";
    if(pl.gravity_lvl > 0)text += "黑洞 <- ["+pl.gravity_lvl+"/"+MaxLevel+"]\n";
    if(pl.ultimate_lvl > 0)text += "终极 <- ["+pl.ultimate_lvl+"/"+MaxLevel+"]\n";
    if(pl.heal_lvl > 0)text += "治疗 <- ["+pl.heal_lvl+"/"+MaxLevel+"]\n";
    if(text == "")text += "没有神器等级";
    return text;
}
...
function PrintText_Info(pl)
{
    ...
    text += "名字 <- "+pl.name+"\n";
    text += "金钱 <- "+pl.money+"\n";
    text += "身份 <- " + ((pl.mapper) ? "Mapper" : (pl.vip) ? "VIP" : "Player\n");
    text += "使用!mc_help 查看相关指令的使用";
...
function PrintText_Perk(pl)
{
    ...
    if(pl.perkhuckster_lvl > 0)
        text += "讨价还价 <- ["+pl.perkhuckster_lvl+"/"+perkhuckster_maxlvl+"]\n";
    if(pl.perksteal_lvl > 0)
        text += "窃贼("+pl.otm+") <- ["+pl.perksteal_lvl+"/"+perksteal_maxlvl+"]\n";
    if(pl.perkluck_lvl > 0)
        text += "幸运战士 <- ["+pl.perkluck_lvl+"/"+perkluck_maxlvl+"]\n";
    if(text == "")
        text += "没有额外能力";
    return text;
}
...
function PrintText_Perk_zm(pl)
{
    ...
    if(pl.perkhp_zm_lvl > 0)
        text += "僵尸血量 <- ["+pl.perkhp_zm_lvl+"/"+400+"]\n";
    if(pl.perkspeed_lvl > 0)
        text += "僵尸速度 <- ["+pl.perkspeed_lvl+"/"+perkspeed_maxlvl+"]\n";
    if(pl.perkchameleon_lvl > 0)
        text += "僵尸隐身 <- ["+pl.perkchameleon_lvl+"/"+perkchameleon_maxlvl+"]\n";
    if(pl.perkresist_zm_lvl > 0)
        text += "人类神器抵抗 <- ["+pl.perkresist_zm_lvl+"/"+50+"]\n";
    if(text == "")
        text += "没有僵尸能力";
    return text;
}
...
function PrintText_Perk_hm(pl)
{
    ...
    if(pl.perkhp_hm_lvl > 0)
        text += "人类血量 <- ["+pl.perkhp_hm_lvl+"/"+400+"]\n";
    if(pl.perkresist_hm_lvl > 0)
        text += "攻击抵抗 <- ["+pl.perkresist_hm_lvl+"/"+perkresist_hm_maxlvl+"]\n";
    if(pl.item_buff_radius || pl.item_buff_last || pl.item_buff_recovery || pl.item_buff_turbo || pl.item_buff_doble)
        text += "辅助神器 <- "+((pl.item_buff_radius) ? "All" : (pl.item_buff_last) ? "Final Attack" : (pl.item_buff_recovery) ? "Recovery" : (pl.item_buff_last) ? "MP turbo" : "W-magic")+"\n";
    if(text == "")
        text += "没有人类能力";
    return text;
}
